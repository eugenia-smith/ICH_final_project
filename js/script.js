const eventsStore = [
  {
    title: "INFJ Personality Type - Coffee Shop Meet & Greet",
    description: "Being an INFJ",
    date: new Date(2024, 2, 23, 15),
    image:
      "https://images.unsplash.com/photo-1541167760496-1628856ab772?q=80&w=1037&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx 8fA%3D%3D%201037w ",
    type: "offline",
    attendees: 99,
    category: "Hobbies and Passions",
    distance: 50,
  },
  {
    title:
      "NYC AI Users - AI Tech Talks, Demo & Social: RAG Search and Customer Experience",
    description: "New York AI Users",
    date: new Date(2024, 2, 23, 11, 30),
    image:
      "https://images.unsplash.com/photo-1696258686454-60082b2c33e2?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8 fA%3D%3D ",
    type: "offline",
    attendees: 43,
    category: "Technology",
    distance: 25,
  },
  {
    title: "Book 40+ Appointments Per Month Using AI and Automation",
    description: "New Jersey Business Network",
    date: new Date(2024, 2, 16, 14),
    image:
      "https://images.unsplash.com/photo-1674027444485-cec3da58eef4?q=80&w=1032&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx 8fA%3D%3D",
    type: "online",
    category: "Technology",
    distance: 10,
  },
  {
    title: "Dump writing group weekly meetup",
    description: "Dump writing group",
    date: new Date(2024, 2, 13, 11),
    image:
      "https://plus.unsplash.com/premium_photo-1678453146992-b80d66df9152?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufD B8fHx8fA%3D%3D",
    type: "online",
    attendees: 77,
    category: "Business",
    distance: 100,
  },
  {
    title: "Over 40s, 50s, & 60s Senior Singles Chat, Meet & Dating Community",
    description: "Over 40s, 50s, 60s Singles Chat, Meet & Dating Community",
    date: new Date(2024, 2, 14, 11),
    image:
      "https://plus.unsplash.com/premium_photo-1706005542509-a460d6efecb0?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufD B8fHx8fA%3D%3D",
    type: "online",
    attendees: 140,
    category: "Social Activities",
    distance: 75,
  },
  {
    title: "All Nations - Manhattan Missions Church Bible Study",
    description: "Manhattan Bible Study Meetup Group",
    date: new Date(2024, 2, 14, 8),
    image:
      "https://plus.unsplash.com/premium_photo-1679488248784-65a638a3d3fc?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufD B8fHx8fA%3D%3D",
    type: "offline",
    category: "Health and Wellbeing",
    distance: 15,
  },
];

// получает значение select'а
function getFilter(name) {
  const elem = document.querySelector(`[name=${name}]`);
  return elem.value;
}

// выводит результаты фильтрации
function showSearchResults() {
  const resultsElem = document.querySelector(".search_results");
  let results = eventsStore;

  // фильтрация по категории
  const categoryFilter = getFilter("category_select");
  if (categoryFilter) {
    results = results.filter((elem) => {
      return elem.category == categoryFilter;
    });
  }

  // фильтрация по расстоянию
  const distanceFilter = getFilter("distance_select");
  if (distanceFilter) {
    results = results.filter((elem) => {
      return elem.distance == distanceFilter;
    });
  }

  // фильтрация по типу
  const typeFilter = getFilter("type_select");
  if (typeFilter) {
    results = results.filter((elem) => {
      return elem.type == typeFilter;
    });
  }

  // фильтрация по дате
  const dateFilter = getFilter("date_select");
  if (dateFilter) {
    results = results.filter((elem) => {
      return elem.date.getMonth() + "," + elem.date.getDate() == dateFilter;
    });
  }

  // отрисовка элементов

  resultsElem.innerHTML = "";

  results.forEach((elem) => {
    const newElem = document.createElement("article");
    newElem.innerHTML =
      (elem.type == "online" ? "<div class='online_mark'></div>" : "") +
      `<picture class='search_option_img'> \
        <img src='${elem.image}' alt='' /> \
       </picture> \
       <div class='search_option_info'> \
        <p class='option_date'>${elem.date.toLocaleDateString(undefined, {
          weekday: "short",
          month: "short",
          day: "numeric",
        })} - ${elem.date.toLocaleTimeString(undefined, {
        hour: "numeric",
        minute: "numeric",
        timeZoneName: "short",
      })}</p> \
        <p class='option_heading'>${elem.title}</p> \
        <p class='option_category'>${elem.category}</p>` +
      (elem.attendees
        ? "<p class='option_attendees'>" + elem.attendees + " attendees</p>"
        : "") +
      "</div>";
    resultsElem.append(newElem);
  });
}

document.addEventListener("DOMContentLoaded", (event) => {
  showSearchResults();
});

const selectArr = document.querySelectorAll(".filter_select");
selectArr.forEach((elem) => {
  elem.addEventListener("change", (event) => {
    showSearchResults();
  });
});
